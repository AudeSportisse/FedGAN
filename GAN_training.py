...
from util_tools import get_time_stamp, mkdir, sample_Z, set_lr
from util_tools import loss_plot


class GANUpdate(object):
    ...
    def independent_training(self, G, D, dataset, station_name='', save_pth_pre=''):
        ...
        with tqdm(range(self.args.epochs)) as tq:
            for j in tq:  # 暂时取消self.args.local_ep *
                ...
                if self.args.input_dim == self.Dim:
                    New_X_mb = M_mb * X_mb + (1 - M_mb) * Z_mb  # Missing Data Introduce
                else:
                    New_X_mb = Z_mb  # Missing Data Introduce
                ...
                if self.args.lr_decay:
                    g_frac = j // self.args.g_lr_decay_step
                    decay_factor = self.args.g_lr_decay ** g_frac
                    current_g_lr = self.args.g_lr * decay_factor
                    set_lr(optimizer_G, lr=current_g_lr)

                    d_frac = j // self.args.d_lr_decay_step
                    decay_factor = self.args.d_lr_decay ** d_frac
                    current_d_lr = self.args.d_lr * decay_factor
                    set_lr(optimizer_D, lr=current_d_lr)

                if self.args.use_amp:
                    optimizer_D.zero_grad()
                    with autocast():
                        D_loss_curr = self.compute_D_loss(G, D, M=M_mb, X=X_mb, New_X=New_X_mb, H=H_mb)
    ...
    def compute_D_loss(self, G, D, M, X, New_X, H):
        """
        :arg
        M: mask
        New_X: 缺失后的数据
    ...
    def sample_idx(self, m, n):
        ...
        if self.idt != 0:
            if self.idt > m // self.idt:
                self.idt = 0
                idx = np.arange(n) + self.idt * n
            else:
                idx = np.arange(n) + self.idt * n
                self.idt += 1
        else:
    ...
    def testing(self, G, mode_name=''):
        ...
        # 对算法进行测试
        Z_mb = sample_Z(self.Test_No, self.args.input_dim)
        M_mb = self.testM
        X_mb = self.testX
